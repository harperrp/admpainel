...
if ($method === 'GET') {
    ...
    $sql = $admin
      ? 'SELECT * FROM laws ORDER BY publication_date DESC, id DESC'
      : "SELECT * FROM laws WHERE status = 'published' ORDER BY publication_date DESC, id DESC";
...
if ($method === 'POST') {
    ...
    $stmt->execute([
      $d['title'] ?? '', $d['law_number'] ?? null, $d['law_type'] ?? null, $d['summary'] ?? null,
      $d['content'] ?? null, $d['file_url'] ?? null, $d['publication_date'] ?? date('Y-m-d'), $d['status'] ?? 'draft'
    ]);
...
if ($method === 'PUT') {
    ...
    $stmt->execute([
      $d['title'] ?? '', $d['law_number'] ?? null, $d['law_type'] ?? null, $d['summary'] ?? null,
      $d['content'] ?? null, $d['file_url'] ?? null, $d['publication_date'] ?? date('Y-m-d'), $d['status'] ?? 'draft', $id
    ]);
